<div>
    <x-layout.page-header title="Laporan Nilai" subtitle="Cetak laporan hasil belajar siswa di kelas yang Anda walikan ke PDF">
        <x-slot:actions>
            ...
            <div class="d-flex flex-column flex-md-row gap-2 align-items-md-center mb-3">
                ...
                @if($selectedMapelId && $mapel)
                    <x-ui.button variant="danger" icon="fas fa-file-pdf"
                        href="{{ route('guru.laporan-nilai.cetak-mapel', ['kelas' => $selectedKelasId, 'mapel' => $selectedMapelId, 'tahun' => $selectedTahunId]) }}"
                        target="_blank">
                        Cetak PDF {{ $mapel }}
                    </x-ui.button>
                @endif
    ...
    @else
        <x-layout.modern-card>
            <x-ui.empty-state
                ...
                description="Silakan pilih kelas perwalian dan tahun ajaran di sudut kanan atas untuk melihat daftar siswa."
            />
        </x-layout.modern-card>
    @endif
</div>
